...
static mesa_rc forward_frame_to_cpu_method1(BOOL enable); 
static mesa_rc forward_frame_to_cpu_method2(BOOL enable);
static mesa_rc forward_frame_to_cpu_method3(BOOL enable);
...
static mesa_rc forward_frame_to_cpu_method3(BOOL enable)
{
    ...
    // (1) --- IPv4 entry
    if (acl_mgmt_ace_init(VTSS_ACE_TYPE_IPV4, &conf) != MESA_RC_OK) {
        return rc;
    }
    ...
    if (acl_mgmt_ace_add(ACL_USER_NAT, ACL_MGMT_ACE_ID_NONE, &conf) != MESA_RC_OK) {
