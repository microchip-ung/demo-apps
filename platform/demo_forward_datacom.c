...
static vtss_rc forward_frame_to_cpu_method2(BOOL enable)
{
    ...
    if (rc != VTSS_OK) {
        T_EG(TRACE_GRP_FORWARD, "vtss_mac_table_add() failed (%s)", error_txt(rc));
    }
...
// --- In the following function, you need to define ACL_USER_DATACOM in 
//     the type  acl_user_t  of file vtss_appl/acl/acl_api.h
//
static vtss_rc forward_frame_to_cpu_method3(BOOL enable)
{
    ...
    // (1) --- IPv4 entry
    rc = acl_mgmt_ace_init(VTSS_ACE_TYPE_IPV4, &conf);
    if (rc) return rc;

    // --- A rule need an ID which is any number. 
    conf.id = 1;
    ...
    rc = acl_mgmt_ace_add(ACL_USER_NAT, ACL_MGMT_ACE_ID_NONE, &conf);
